...
function createUser()
{
    ...
    if (isset($_POST['submit']))
    {
        ...
        $query->execute();
        header("Location: backend.php?status=added");
    }
    else
        {
            header("Location: backend.php?status=0");
        }
}
...
function updateUser()
{
    ...
    if (isset($_POST['entryID']) && isset($_POST['submit']))
    {
        ...
        $query->execute();
        header("Location: backend.php?status=updated&ID=$entryID");
    }
    else
        {
            header("Location: backend.php?status=0");
...
function deleteUser() {
    ...
    if (isset($_GET['ID'])) {
        ...
        $query->execute();
        header("Location: backend.php?status=deleted&ID=$entryID");
    }
    else
    {
        header("Location: backend.php?status=0");
